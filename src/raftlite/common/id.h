...
namespace std {
template <>
struct hash<metable::NodeID> {
    std::size_t operator()(const metable::NodeID &n) const noexcept {
        return std::hash<std::string>()(n.ToBinary());
    }
};

template <>
struct hash<metable::TermID> {
    std::size_t operator()(const metable::TermID &t) const noexcept {
